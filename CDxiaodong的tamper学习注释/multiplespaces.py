...
def tamper(payload, **kwargs):
    ...
    if payload:
        ...
        for match in re.finditer(r"\b[A-Za-z_]+\b", payload):
            #re.finditer正则匹配的方式之一，返回一个迭代器，该迭代器在string类型的RE 模式的所有非重叠匹配中产生匹配对象。
            #  从左到右扫描该字符串，并以找到的顺序返回匹配项。空匹配项包含在结果中。
            word = match.group()
        ...
        for word in words:
            ...
            #random.randint(1, 4)  将空格与1~4之间相乘带入，然后将retVal调入到word函数去拼接
