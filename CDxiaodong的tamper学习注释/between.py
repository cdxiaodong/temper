...
def tamper(payload, **kwargs):
    ...
    if payload:
        ...
        if match:
            ...
            ##分别将match中的第一个字符串、第三个字符串、第四个字符串进行填入
            ##group（0）和group（）一样。整个就是用retVal用于整型输出
        else:
            retVal = re.sub(r"\s*>\s*(\d+|'[^']+'|\w+\(\d+\))", r" NOT BETWEEN 0 AND \g<1>", payload)
            ##sub消息提示上面那些
        if retVal == payload:
            ##也就是说retVal没有改变，也就是说上面的那些操作并没有对retVel产生作用
            match = re.search(r"(?i)(\b(AND|OR)\b\s+)(?!.*\b(AND|OR)\b)([^=]+?)\s*=\s*([\w()]+)\s*", payload)
            ##重新match匹配，不过有个规则不是>，而是=
            if match:
